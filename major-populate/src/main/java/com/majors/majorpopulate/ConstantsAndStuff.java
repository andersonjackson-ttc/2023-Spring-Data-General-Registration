...
public class ConstantsAndStuff {
    ...
    public static void showMajorRequirements(String nameOfMajor) {
        ...
        try {
            ...
            while(result.next() != false) {
               ...
               majorRequirement.add(new MajorRequirements(result.getString("major_name"), result.getString("req_type"), result.getString("course_id"), getCourseName(result.getString("course_id"))));
            }
    ...
    //Grabs course name from data base with courseID
    private static String getCourseName(String courseID) {
        java.sql.Statement sqlSt; //runs sql
        
        ResultSet result; //holds the output from the sql
        
        String correctCourseID = courseID.substring(1);
        String SQL = "SELECT course_title from tbl_courses_offered where course_section like '" + correctCourseID + "%' LIMIT 1";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            String dbURL = "jdbc:mysql://127.0.0.1:3306/cpt275_db";
            Connection dbConnect = DriverManager.getConnection(dbURL, "root", "root");
            ...
            result = sqlSt.executeQuery(SQL);
            while(result.next() != false) {
             return result.getString("course_title");
            }
            sqlSt.close();
